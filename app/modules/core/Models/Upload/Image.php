...
class Core_Model_Upload_Image
{
    ...
    private $_size = 2000097152; // Peso máximo. MAX_FILE_SIZE sobrescribe este valor
    ...
    public function upload($image)
    {
        if ($_FILES[$image]["error"] > 0) {
            ...
            if (in_array($mime, $this->_exts) && $_FILES[$image]['size'] <= $this->_size) {
                $filename = '' . pathinfo($_FILES[$image]['name'], PATHINFO_FILENAME);
                $filename = $this->clearName($filename);
                $extension = strtolower(pathinfo($_FILES[$image]['name'], PATHINFO_EXTENSION));
                if (in_array($extension, array('php', 'php3', 'php4', 'php5', 'phtml', 'exe', 'sh'))) {
                    return false;
                }
                ...
                if ($ancho_orig > $ancho_max or $alto_orig > $alto_max) {
                    ...
                    switch ($mime) {
                        case "image/jpg":
                        case "image/jpeg":
                            $image_resource = imagecreatefromjpeg($origen);
                            imagecopyresampled($canvas, $image_resource, 0, 0, 0, 0, $ancho_max, $alto_max, $ancho_orig, $alto_orig);
                            imagejpeg($canvas, $ruta, 100);
                            return $name;
                            break;
                        case "image/gif":
                            $image_resource = imagecreatefromgif($origen);
                            imagecopyresampled($canvas, $image_resource, 0, 0, 0, 0, $ancho_max, $alto_max, $ancho_orig, $alto_orig);
                            imagegif($canvas, $ruta);
                            return $name;
                            break;
                        case "image/png":
                            $image_resource = imagecreatefrompng($origen);
                            imagecopyresampled($canvas, $image_resource, 0, 0, 0, 0, $ancho_max, $alto_max, $ancho_orig, $alto_orig);
    ...
    public function uploadmultiple($image)
    {
        ...
        foreach ($_FILES[$image]["tmp_name"] as $key => $tmp_name) {
            $finfo = new finfo(FILEINFO_MIME_TYPE);
            $mime = $finfo->file($_FILES[$image]['tmp_name'][$key]);
            if (in_array($mime, $permitidos) && $_FILES[$image]['size'][$key] <= $limite_kb * 1024) {
