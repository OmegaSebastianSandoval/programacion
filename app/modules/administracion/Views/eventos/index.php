			...
			<div class="row">
				<div class="col-3">
					...
					<label class="input-group">
						...
						<select class="form-control" name="evento_tipo">
							<option value="">Todas</option>
							<?php foreach ($this->list_evento_tipo as $key => $value): ?>
								<option value="<?= $key; ?>" <?php if ($this->getObjectVariable($this->filters, 'evento_tipo') == $key) {
										echo "selected";
									} ?>><?= $value; ?></option>
							<?php endforeach ?>
						</select>
				...
				<div class="col-3">
					...
					<label class="input-group">
						...
						<select class="form-control" name="evento_lugar">
							<option value="">Todas</option>
							<?php foreach ($this->list_evento_lugar as $key => $value): ?>
								<option value="<?= $key; ?>" <?php if ($this->getObjectVariable($this->filters, 'evento_lugar') == $key) {
										echo "selected";
									} ?>><?= $value; ?></option>
							<?php endforeach ?>
						</select>
				...
				<div class="col-3">
					...
					<label class="input-group">
						...
						<select class="form-control" name="evento_estado">
							<option value="">Todas</option>
							<?php foreach ($this->list_evento_estado as $key => $value): ?>
								<option value="<?= $key; ?>" <?php if ($this->getObjectVariable($this->filters, 'evento_estado') == $key) {
										echo "selected";
									} ?>><?= $value; ?></option>
							<?php endforeach ?>
						</select>
			...
			<table class=" table table-striped  table-hover table-administrator text-left">
				...
				<tbody>
					<?php foreach ($this->lists as $content) { ?>
						<?php $id = $content->evento_id; ?>
						<tr>
							<td><?= $this->list_evento_tipo[$content->evento_tipo]; ?>
							<td><?= $content->evento_nombre; ?></td>
							<td>
								<?php if ($content->evento_imagen) { ?>
									<img src="/images/<?= $content->evento_imagen; ?>" class="img-thumbnail thumbnail-administrator" />
								<?php } ?>
								<div><?= $content->evento_imagen; ?></div>
							</td>
							<td><?= $content->evento_costo; ?></td>
							<td><?= $content->evento_fecha; ?></td>
							<td><?= $this->list_evento_lugar[$content->evento_lugar]; ?>
							<td><?= $content->evento_bono == 1 ? 'Sí' : 'No'; ?></td>
							<td><?= $content->evento_activo == 1 ? 'Sí' : 'No'; ?></td>
							<td><?= $this->list_evento_estado[$content->evento_estado]; ?>
							<td>
